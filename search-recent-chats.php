...
if(isset($_POST['search'])){
    $search = $_POST['search'];
    $sql = "SELECT DISTINCT users.* FROM users JOIN messages ON messages.incoming_msg_id = users.ID OR messages.outgoing_msg_id = users.ID WHERE ((messages.incoming_msg_id = $outgoing_id OR messages.outgoing_msg_id = $outgoing_id) AND (users.ID != $outgoing_id OR (messages.incoming_msg_id = $outgoing_id AND messages.outgoing_msg_id = $outgoing_id)) AND users.Username LIKE '%$search%') ORDER BY (
        CASE
